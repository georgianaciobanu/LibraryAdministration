...
using SimbioMed.Migrations.Seed.Host;
...
namespace SimbioMed.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : SimbioMedAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<SimbioMedDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{SimbioMedConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{SimbioMedConsts.ConnectionStringName}", constring);
        }
