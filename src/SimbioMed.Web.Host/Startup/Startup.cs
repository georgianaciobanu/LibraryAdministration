...
using SimbioMed.Identity;
using SimbioMed.Web.Chat.SignalR;
using SimbioMed.Web.Common;
using Swashbuckle.AspNetCore.Swagger;
using SimbioMed.Web.IdentityServer;
using SimbioMed.Web.Swagger;
using Stripe;
...
using SimbioMed.Schemas;
using SimbioMed.Web.HealthCheck;
using Newtonsoft.Json.Serialization;
using Owl.reCAPTCHA;
using HealthChecksUISettings = HealthChecks.UI.Configuration.Settings;
using Microsoft.AspNetCore.Server.Kestrel.Https;

namespace SimbioMed.Web.Startup
{
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            ...
            return services.AddAbp<SimbioMedWebHostModule>(options =>
            {
...
app.Use(async (context, next) =>                {                    await next();                    if (context.Response.StatusCode == 404                        && !Path.HasExtension(context.Request.Path.Value))                    {                        context.Request.Path = "/index.html";                        await next();                    }                });
            ...
            if (WebConsts.SwaggerUiEnabled)
            {
                ...
                {
                    options.SwaggerEndpoint(_appConfiguration["App:SwaggerEndPoint"], "SimbioMed API V1");
                    options.IndexStream = () => Assembly.GetExecutingAssembly()
                        .GetManifestResourceStream("SimbioMed.Web.wwwroot.swagger.ui.index.html");
        ...
        private void ConfigureSwagger(IServiceCollection services)
        {
            ...
            {
                options.SwaggerDoc("v1", new OpenApiInfo() {Title = "SimbioMed API", Version = "v1"});
                ...
                if (canShowSummaries)
                {
                    ...
                    var applicationXml = $"SimbioMed.Application.xml";
                    var applicationXmlPath = Path.Combine(AppContext.BaseDirectory, applicationXml);
                    options.IncludeXmlComments(applicationXmlPath);

                    var webCoreXmlFile = $"SimbioMed.Web.Core.xml";
